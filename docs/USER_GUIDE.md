...
Outcome colors in History: a pending recall is recorded as a yellow **warning** (the activation was missed but a retry is armed), and it only turns into a red **error** in a final entry if the recall expires or gives up. Successful retries append a green ok entry.
